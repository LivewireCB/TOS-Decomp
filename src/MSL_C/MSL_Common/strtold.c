...
// long double __strtold(int max_width, int (*ReadProc)(void *, int, int), void*
// ReadProcArg, int* chars_scanned, int* overflow) { 	int scan_state = 1; 	int
// hex_scan_state	= 0; 	int count = 0; 	int spaces = 0; 	int c; 	decimal d =
// {0, 0, 0, {0, ""}}; 	int sig_negative = 0; 	int exp_negative = 0; 	long
// exp_value = 0; 	int exp_adjust = 0; 	register long double result = 0.0; 	int
// sign_detected	= 0; 	int radix_marker = *(unsigned
// char*)(__lconv).decimal_point;
...
// 					*chars_scanned = spaces + i +
// sign_detected; 					return result;
// 				}
// 				else {
// 					scan_state = 4096;
// 				}
...
// (c == radix_marker))) { 							nan_arg[j++] = (char)c; 							c = fetch();
// 						}
...
// 				}
// 				if (d.sig.length < 20) {
// 					d.sig.text[d.sig.length++] = (unsigned
// char)c;
// 				}
// 				else {
// 					exp_adjust++;
...
// 					case 1:
// 						memset(mantissa, 0,
// sizeof(mantissa)); 						chptr			= mantissa; 						mantissa_digits =
// (53 + 3) / 4; 						intdigits		= 0; 						NibbleIndex		= 0;
// 						hex_scan_state = 2;
// 						c = fetch();
// 						break;
...
// { 							intdigits++; 							uch = *(chptr + NibbleIndex / 2);
...
// 						if (intdigits < mantissa_digits)
// { 							uch = *(chptr + NibbleIndex / 2); 							ui = toupper(c);
...
// 	if (scan_state != 32768 ? !success(scan_state) : !hex_success(count,
// hex_scan_state)) { 		count = 0; 		*chars_scanned = 0;
// 	}
// 	else {
// 		count--;
...
// chptr--) { 				unsigned char a = *chptr; 				*chptr = (unsigned char)((a << one_bit)
// | carry); 				carry = (unsigned char)(a >> (8 - one_bit));
// 			}
// 		}
...
// 			dbl_bit += 8;
// 			dbl_bits[dbl_bit>>3] |= (unsigned char)(ui << (8 -
// halfbits));
// 		}
