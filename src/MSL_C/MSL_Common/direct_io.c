...
// size_t __fwrite(const void *pPtr, size_t memb_size, size_t num_memb, FILE
// *pFile) {
...
//     buff = (!pFile->mode.binary || pFile->mode.buffer == 2 ||
//     pFile->mode.buffer == 1);
...
//         pFile->buffer_len = pFile->buffer_size - (pFile->buffer_ptr -
//         pFile->buffer);
