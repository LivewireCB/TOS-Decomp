...
void __OSGetIOSRev(OSIOSRev *rev) {
  ...
  rev->buildYear = (builddate >> 0 & 0xF) + (builddate >> 4 & 0xF) * 10 + 2000;
}
...
static void ClearArena(void) DECOMP_DONT_INLINE {
  ...
  if (!((OSGetResetCode() >> 31) & 1)) {
    MemClear(OSGetMEM1ArenaLo(),
             (u32)OSGetMEM1ArenaHi() - (u32)OSGetMEM1ArenaLo());
  }
  ...
  // Region begins after arena begins
  else if (OSGetMEM1ArenaLo() < __OSRebootParams.regionStart) {
    // Region is outside of arena
    if (OSGetMEM1ArenaHi() <= __OSRebootParams.regionStart) {
      MemClear(OSGetMEM1ArenaLo(),
               (u32)OSGetMEM1ArenaHi() - (u32)OSGetMEM1ArenaLo());
    }
    // Region begins in arena
      ...
      MemClear(OSGetMEM1ArenaLo(),
               (u32)__OSRebootParams.regionStart - (u32)OSGetMEM1ArenaLo());

      // Region ends inside arena
      if (OSGetMEM1ArenaHi() > __OSRebootParams.regionEnd) {
        // Clear arena area after region
        MemClear(__OSRebootParams.regionEnd,
                 (u32)OSGetMEM1ArenaHi() - (u32)__OSRebootParams.regionEnd);
      }
    }
  }
}
...
static void ClearMEM2Arena(void) DECOMP_DONT_INLINE {
  ...
  // Region begins after arena begins
    ...
    // Region begins in arena
      ...
      // Clear arena area before region
      MemClear(OSGetMEM2ArenaLo(),
               (u32)__OSRebootParams.regionStart - (u32)OSGetMEM2ArenaLo());
      // Region ends inside arena
      if (OSGetMEM2ArenaHi() > __OSRebootParams.regionEnd) {
        // Clear arena area after region
        MemClear(__OSRebootParams.regionEnd,
                 (u32)OSGetMEM2ArenaHi() - (u32)__OSRebootParams.regionEnd);
      }
    }
  }
}
...
u32 __OSGetDIConfig(void) { return DI_HW_REGS[DI_CONFIG] & 0xFF; }
