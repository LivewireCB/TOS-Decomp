...
#include "revolution\BTE\stack\btm\btm_ble_int.h"
...
typedef struct {
  ...
  UINT16 restore_pkt_types; /* when coming in/out of SCO connection restore the
                               packet types */
  UINT16 clock_offset;
  BD_ADDR remote_addr;
  DEV_CLASS remote_dc;
...
#define BTM_RMT_NAME_INQ 0x4 /* Remote name initiated internally by inquiry */
  ...
  UINT8
  pending_filt_complete_event; /* to take care of btm_event_filter_complete
                                  corresponding to */
...
#define BTM_SEC_STATE_DELAY_FOR_ENC                                            \
  ...
  BOOLEAN link_key_not_sent; /* link key notification has not been sent waiting
                                for name */
