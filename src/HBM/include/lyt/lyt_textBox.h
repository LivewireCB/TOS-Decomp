...
public:
  ...
  const wchar_t *GetStringBuffer() const { return mTextBuf; }

  const ut::Font *GetFont() const;
  void SetFont(const ut::Font *pFont);

  ut::Color GetTextColor(u32 idx) const { return mTextColors[idx]; }
  void SetTextColor(u32 idx, ut::Color color) { mTextColors[idx] = color; }
  ...
  ut::WideTagProcessor *GetTagProcessor() const { return mpTagProcessor; }
