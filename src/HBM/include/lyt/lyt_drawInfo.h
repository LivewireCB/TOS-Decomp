...
public:
  ...
  void SetViewMtx(const math::MTX34 &rViewMtx) { mViewMtx = rViewMtx; }

  const ut::Rect &GetViewRect() const { return mViewRect; }
  void SetViewRect(const ut::Rect &rViewRect) { mViewRect = rViewRect; }
  ...
  void SetLocationAdjust(bool adjust) { mFlag.locationAdjust = adjust; }
