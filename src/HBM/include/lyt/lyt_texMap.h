...
public:
  ...
  void Set(const TexMap &rOther) { *this = rOther; }
  ...
  void SetImage(void *pImage) { mpImage = pImage; }

  void *GetPalette() const { return mpPalette; }
  void SetPalette(void *pPalette) { mpPalette = pPalette; }
  ...
  void SetLODBias(f32 bias) { mLODBias = static_cast<u16>(bias * 256.0f); }

  u16 GetPaletteEntryNum() const { return mPaletteEntryNum; }
  void SetPaletteEntryNum(u16 entrynum) { mPaletteEntryNum = entrynum; }

  GXTexFmt GetTexelFormat() const {
    return static_cast<GXTexFmt>(mBits.textureFormat);
  }
  void SetTexelFormat(GXTexFmt format) { mBits.textureFormat = format; }
  ...
  void SetPaletteFormat(GXTlutFmt format) { mBits.paletteFormat = format; }
