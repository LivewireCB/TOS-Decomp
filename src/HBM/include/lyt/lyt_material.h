...
public:
  ...
  GXColorS10 GetTevColor(u32 idx) { return mTevCols[idx]; }
  void SetTevColor(u32 idx, const GXColorS10 &rColor) {
    mTevCols[idx] = rColor;
  }

  ut::Color GetTevKColor(u32 idx) { return mTevKCols[idx]; }
  ...
  u8 GetTevStageNum() const { return mGXMemNum.tevStage; }
  ...
  const TexMap &GetTexture(u8 idx) const { return GetTexMapAry()[idx]; }
  ...
  const TexSRT &GetTexSRT(u32 idx) const { return GetTexSRTAry()[idx]; }
  ...
  ut::Color GetMatColor() const { return GetMatColAry()[0]; }
