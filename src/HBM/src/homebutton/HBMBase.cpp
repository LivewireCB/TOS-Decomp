...
#include <MSL_C/include/new> // placement new
...
#include "lyt/lyt_arcResourceAccessor.h"
#include "lyt/lyt_common.h" // nw4hbm::lyt::Size
#include "lyt/lyt_drawInfo.h"
#include "lyt/lyt_group.h"
#include "lyt/lyt_layout.h"
#include "lyt/lyt_material.h"
#include "lyt/lyt_pane.h"
#include "lyt/lyt_textBox.h"
#include "math/math_triangular.h" // nw4hbm::math::Atan2Deg
#include "math/math_types.h"
#include "ut/ut_LinkList.h" // IWYU pragma: keep (NW4HBM_RANGE_FOR)
#include "ut/ut_Rect.h"
#include "ut/ut_ResFont.h"
#include "ut/ut_RuntimeTypeInfo.h" // nw4hbm::ut::DynamicCast
...
extern void __deadstrip1() { (void)(GXColor){0, 0, 0, 0xff}; }
...
void HomeButton::calc(const HBMControllerData *pController) {
  ...
  case 20:
    if (mSelectAnmNum == 4) {
      ...
      if (mAdjustFlag) {
        pJpgPane->SetScale(nw4hbm::math::VEC2(1.0f / mpHBInfo->adjust.x,
                                              1.0f / mpHBInfo->adjust.y));
      } else {
        pJpgPane->SetScale(nw4hbm::math::VEC2(1.0f, 1.0f));
      }

      GXInitTexObj(&mTexObj, mpHBInfoEx->texImage, mpHBInfoEx->texImageWidth,
                   mpHBInfoEx->texImageHeight, mpHBInfoEx->texImageFormat,
  ...
  for (i = 0; i < WPAD_MAX_CONTROLLERS; i++) {
    if (pController->wiiCon[i].kpad) {
      if (pController->wiiCon[i].kpad->wpad_err != WPAD_ENODEV) {
        if (mPadDrawTime[i] >
            static_cast<int>(
                5.0f / getInstance()->getHBMDataInfo()->frameDelta + 0.5f)) {
      ...
      if (mControllerFlag[i]) {
        ...
        for (int j = 0; j < (int)ARRAY_LENGTH(scBatteryPaneName[i]); j++) {
          mpLayout->GetRootPane()
...
void HomeButton::updateTrigPane() {
  ...
  case eSeq_Normal:
    for (i = 0; i < (int)ARRAY_LENGTH(scFuncTouchPaneName); i++) {
      if (i < 2 || i == 9) {
        nw4hbm::lyt::Pane *pTouchPane = mpLayout->GetRootPane()->FindPaneByName(
...
bool HomeButton::getVibFlag() { return WPADIsMotorEnabled(); }
...
void HomeButton::createInfoEx(const HBMDataInfoEx *pHBInfoEx) {
  ...
  for (int i = 0; i < (int)ARRAY_LENGTH(mpAnmControllerSet4); i++) {
    ...
    NW4HBM_RANGE_FOR(it, rPaneLink) {
      it->mTarget->BindAnimation(mpAnmControllerSet4[i]->mpAnimGroup, false);
    }

    f32 frameSize = mpAnmControllerSet4[i]->mpAnimGroup->GetFrameSize();

    mpAnmControllerSet4[i]->init(FrameController::eAnmType_Forward, frameSize,
