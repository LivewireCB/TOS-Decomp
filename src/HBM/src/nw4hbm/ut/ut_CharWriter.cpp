...
void CharWriter::LoadTexture(const Glyph &rGlyph, GXTexMapID slot) {
  ...
  if (loadingTexture != mLoadingTexture) {
    GXTexObj texObj;
    GXInitTexObj(&texObj, rGlyph.pTexture, rGlyph.texWidth, rGlyph.texHeight,
                 rGlyph.texFormat, GX_CLAMP, GX_CLAMP, FALSE);

    GXInitTexObjLOD(&texObj, mFilter.atSmall, mFilter.atLarge, 0.0f, 0.0f, 0.0f,
