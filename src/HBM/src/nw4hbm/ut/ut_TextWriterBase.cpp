...
TextWriterBase<T>::TextWriterBase()
    : mCharSpace(0.0f), mWidthLimit(NW4R_MATH_FLT_MAX), mLineSpace(0.0f),
      mTabWidth(4), mDrawFlag(0), mTagProcessor(&mDefaultTagProcessor) {}
  ...
  while (static_cast<const T *>(reader.GetCurrentPos()) - pStr <= len) {
    if (ch < ' ') {
      ...
      if (oper == TagProcessor::OPERATION_NEXT_LINE) {
        ...
        if (useLimit) {
          pPrevNewLine = static_cast<const T *>(reader.GetCurrentPos());
        }
      ...
      if (useLimit && pPrevStream != pPrevNewLine) {
        ...
        f32 width = IsWidthFixed() ? GetFixedWidth()
