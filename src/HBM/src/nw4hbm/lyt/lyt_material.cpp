...
inline u32 GetTexMtxIdx(u32 mtx) { return (mtx - GX_TEXMTX0) / 3; }
inline u32 GetTexMtx(u32 idx) { return GX_TEXMTX0 + idx * 3; }
...
void SetIndTexMtx(GXIndTexMtxID id, const f32 mtx[2][3]) {
  ...
  if (a00 >= 1.0f || a01 >= 1.0f || a02 >= 1.0f || a10 >= 1.0f || a11 >= 1.0f ||
      a12 >= 1.0f) {
...
Material::Material(const res::Material *pRes, const ResBlockSet &rBlockSet) {
  ...
  ReserveGXMem(texMapNum, texSrtNum, texCoordGenNum, tevStageNum, allocTevSwap,
               indStageNum, indTexSrtNum, allocChanCtrl, allocMatCol,
               allocAlphaComp, allocBlendMode);
  ...
  if (mpGXMem != NULL) {
    ...
    if (texMapNum > 0) {
      const res::Texture *const pTexture =
          detail::ConvertOffsToPtr<res::Texture>(rBlockSet.pTextureList,
...
void Material::ReserveGXMem(u8 texMapNum, u8 texSrtNum, u8 texCoordGenNum,
  ...
  if (mGXMemNum.tevSwap != 0) {
    ...
    pTevSwap[GX_TEV_SWAP0].Set(GX_CH_RED, GX_CH_GREEN, GX_CH_BLUE, GX_CH_ALPHA);

    pTevSwap[GX_TEV_SWAP1].Set(GX_CH_RED, GX_CH_RED, GX_CH_RED, GX_CH_ALPHA);

    pTevSwap[GX_TEV_SWAP2].Set(GX_CH_GREEN, GX_CH_GREEN, GX_CH_GREEN,
                               GX_CH_ALPHA);

    pTevSwap[GX_TEV_SWAP3].Set(GX_CH_BLUE, GX_CH_BLUE, GX_CH_BLUE, GX_CH_ALPHA);
  }
...
const TevSwapMode *Material::GetTevSwapAry() const {
  return detail::ConvertOffsToPtr<TevSwapMode>(mpGXMem,
                                               CalcOffsetTevSwapAry(mGXMemCap));
}
...
bool Material::SetupGX(bool modulate, u8 alpha) {
  ...
  if (IsTevSwapCap()) {
    ...
    for (int i = 0; i < GX_MAX_TEVSWAP; i++) {
      GXSetTevSwapModeTable(static_cast<GXTevSwapSel>(i), pTevSwap[i].GetR(),
                            pTevSwap[i].GetG(), pTevSwap[i].GetB(),
                            pTevSwap[i].GetA());
    }
  } else {
