...
Window::Window(const res::Window *pRes, const ResBlockSet &rBlockSet)
  ...
  if (pMaterialBuf != NULL) {
    const res::Material *const pResMaterial =
        detail::ConvertOffsToPtr<res::Material>(
            rBlockSet.pMaterialList, pMatOffsetTbl[pResContent->materialIdx]);
  ...
  if (pRes->frameNum > 0) {
    ...
    if (mFrames != NULL) {
      ...
      const u32 *const pFrmOffsetTbl =
          detail::ConvertOffsToPtr<u32>(pRes, pRes->frameOffsetTableOffset);

      for (int i = 0; i < mFrameNum; i++) {
        const res::WindowFrame *const pResFrame =
            detail::ConvertOffsToPtr<res::WindowFrame>(pRes, pFrmOffsetTbl[i]);
...
ut::Color Window::GetVtxColor(u32 idx) const { return mContent.vtxColors[idx]; }
...
#define DRAW_FRAME(FRAME, FLIP)                                                \
  ...
  Get##FRAME##TexCoord(texCoords[0], size, texSize, FLIP);                     \
                                                                               \
  detail::DrawQuad(point, size, 1, texCoords, useVtxColor ? vtxColors : NULL,  \
                   alpha);
...
#define DRAW_FRAME(FRAME)                                                      \
  if (pFrames[WINDOWFRAME_##FRAME].pMaterial->GetTextureNum() > 0) {           \
    bool useVtxColor =                                                         \
        pFrames[WINDOWFRAME_##FRAME].pMaterial->SetupGX(modVtxColor, alpha);   \
                                                                               \
    Get##FRAME##FrameSize(&point, &size, rBase, mSize, rFrameSize);            \
                                                                               \
    Get##FRAME##TexCoord(                                                      \
...
WindowFrameSize Window::GetFrameSize(u8 frameNum, const Frame *pFrames) {
  ...
  switch (frameNum) {
  case 1: {
    Size texSize = detail::GetTextureSize(pFrames[WINDOWFRAME_LT].pMaterial, 0);
