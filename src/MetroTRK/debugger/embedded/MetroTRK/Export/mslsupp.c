#include "MetroTRK\include\portable\mslsupp.h"
#include "MetroTRK\include\TRK_Types.h"
#include <MSL_C/include/size_t.h>

inline IOError __access_file(u32 handle, u8 *buffer, size_t *count,
                             __ref_con ref_con, u8 read);
...
IOError __TRK_write_console(u32 handle, u8 *buffer, size_t *count,
                            __ref_con ref_con) {
...
inline IOError __write_file(u32 handle, u8 *buffer, size_t *count,
                            __ref_con ref_con) {
...
/* there are odd issues going on with __read_file but I don't care enough to
 * figure out what. it matches otherwise */
IOError __read_console(u32 handle, u8 *buffer, size_t *count,
                       __ref_con ref_con) {
  if ((GetUseSerialIO() & 0xFF) == 0) {
    return 1;
  }
