...
#include "MetroTRK\include\portable/mem_TRK.h"
...
signed int TRKGetNextEvent(NubEvent *resultEvent) {
  s32 result = 0;

  TRKAcquireMutex(&gTRKEventQueue.fMutex);

  if (gTRKEventQueue.fCount > 0) {
    TRKCopyEvent(resultEvent,
                 &gTRKEventQueue.fEventList[gTRKEventQueue.fFirst]);
    gTRKEventQueue.fCount--;
