...
#include "MetroTRK\include\dolphin.h"
#include "MetroTRK\include\portable/dserror.h"
#include "MetroTRK\include\portable/mpc_7xx_603e.h"
#include "MetroTRK\include\portable/msgbuf.h"
#include "MetroTRK\include\portable/msgcmd.h"
#include "MetroTRK\include\portable/nubevent.h"
#include "MetroTRK\include\portable/ppc_mem.h"
#include "MetroTRK\include\portable/ppc_reg.h"
#include "MetroTRK\include\portable/support.h"
#include "MetroTRK\include\portable/targimpl_ppc.h"

static void TRKExceptionHandler(ui16);
DSError TRKTargetAccessMemory(void *Data, void *virtualAddress,
                              size_t *memorySize,
                              MemoryAccessOptions accessOptions, s32 read);
...
static PpcStepStatus gTRKStepStatus = {0, kDSStepIntoCount, 0, 0,
                                       0, {0, 0, 0, 0}};
...
ProcessorRestoreFlags_PPC gTRKRestoreFlags = {0, 0, {0, 0, 0, 0, 0, 0, 0}};
...
** Bit 31        :        0
*/
#define INSTR_MTSPR(SPR, rS) /* Bit  0.. 5 */                                  \
  (SHIFT_LEFT_X_BY_Y_BITS(31, 26)) |                                           \
      /* Bit  6..10 */ (SHIFT_LEFT_X_BY_Y_BITS(rS, 21)) |                      \
      /* Bit 11..20 */ (SHIFT_LEFT_X_BY_Y_BITS((SPR & 0xfe0), 6)) |            \
      (SHIFT_LEFT_X_BY_Y_BITS((SPR & 0x1f), 16)) |                             \
      /* Bit 21..30 */ (SHIFT_LEFT_X_BY_Y_BITS(467, 1)) | /* Bit     31 */ 0
...
** Bit 16..31    :        d
*/
#define INSTR_STW(rS, d, rA) /* Bit  0.. 5 */                                  \
  (SHIFT_LEFT_X_BY_Y_BITS(36, 26)) |                                           \
      /* Bit  6..10 */ (SHIFT_LEFT_X_BY_Y_BITS(rS, 21)) |                      \
      /* Bit 11..15 */ (SHIFT_LEFT_X_BY_Y_BITS(rA, 16)) | /* Bit 16..31 */ d
...
** Bit 16..31    :        d
*/
#define INSTR_LWZ(rD, d, rA) /* Bit  0.. 5 */                                  \
  (SHIFT_LEFT_X_BY_Y_BITS(32, 26)) |                                           \
      /* Bit  6..10 */ (SHIFT_LEFT_X_BY_Y_BITS(rD, 21)) |                      \
      /* Bit 11..15 */ (SHIFT_LEFT_X_BY_Y_BITS(rA, 16)) | /* Bit 16..31 */ d
...
** Bit 16..31    :        d
*/
#define INSTR_STFD(frS, d, rA) /* Bit  0.. 5 */                                \
  (SHIFT_LEFT_X_BY_Y_BITS(54, 26)) |                                           \
...
** Bit 16..31    :        d
*/
#define INSTR_LFD(frD, d, rA) /* Bit  0.. 5 */                                 \
  (SHIFT_LEFT_X_BY_Y_BITS(50, 26)) |                                           \
...
** Bit 31        :        0
*/
#define INSTR_MFFS(frD) /* Bit  0.. 5 */                                       \
  (SHIFT_LEFT_X_BY_Y_BITS(63, 26)) |                                           \
      /* Bit  6..10 */ (SHIFT_LEFT_X_BY_Y_BITS(frD, 21)) |                     \
      /* Bit 11..20 */ 0 | /* Bit 21..30 */ (SHIFT_LEFT_X_BY_Y_BITS(583, 1)) | \
      /* Bit     31 */ 0
...
** Bit 31        :        0
*/
#define INSTR_MTFSF(FM, frB) /* Bit  0.. 5 */                                  \
  (SHIFT_LEFT_X_BY_Y_BITS(63, 26)) | /* Bit  6     */ 0 |                      \
      /* Bit  7..14 */ (SHIFT_LEFT_X_BY_Y_BITS(FM, 17)) | /* Bit  15    */ 0 | \
      /* Bit 16..20 */ (SHIFT_LEFT_X_BY_Y_BITS(frB, 11)) |                     \
      /* Bit 21..30 */ (SHIFT_LEFT_X_BY_Y_BITS(711, 1)) | /* Bit     31 */ 0
...
** Bit 31        :        0
*/
#define INSTR_LVX(vD, rA, rB) /* Bit  0.. 5 */                                 \
  (SHIFT_LEFT_X_BY_Y_BITS(31, 26)) |                                           \
      /* Bit  6..10 */ (SHIFT_LEFT_X_BY_Y_BITS(vD, 21)) |                      \
      /* Bit 11..15 */ (SHIFT_LEFT_X_BY_Y_BITS(rA, 16)) |                      \
      /* Bit 16..20 */ (SHIFT_LEFT_X_BY_Y_BITS(rB, 11)) |                      \
      /* Bit 21..30 */ (SHIFT_LEFT_X_BY_Y_BITS(103, 1)) | /* Bit 31     */ 0
...
** Bit 31        :        0
*/
#define INSTR_MFVSCR(vD) /* Bit  0.. 5 */                                      \
  (SHIFT_LEFT_X_BY_Y_BITS(4, 26)) |                                            \
      /* Bit  6..10 */ (SHIFT_LEFT_X_BY_Y_BITS(vD, 21)) |                      \
      /* Bit 11..20 */ (SHIFT_LEFT_X_BY_Y_BITS(0, 0)) |                        \
      /* Bit 21..30 */ (SHIFT_LEFT_X_BY_Y_BITS(1540, 0)) | /* Bit 31     */ 0
...
** Bit 31        :        0
*/
#define INSTR_MTVSCR(vB) /* Bit  0.. 5 */                                      \
  (SHIFT_LEFT_X_BY_Y_BITS(4, 26)) |                                            \
      /* Bit  6..10 */ (SHIFT_LEFT_X_BY_Y_BITS(vB, 21)) |                      \
      /* Bit 11..20 */ (SHIFT_LEFT_X_BY_Y_BITS(0, 0)) |                        \
      /* Bit 21..30 */ (SHIFT_LEFT_X_BY_Y_BITS(1604, 0)) | /* Bit 31     */ 0
...
void TRKTargetSetStopped(s32 stopped) { gTRKState.stopped = stopped; }

s32 TRKTargetStopped() { return gTRKState.stopped; }
...
DSError TRKTargetSupportRequest(void) {
  ...
  if ((command != kDSReadFile) && (command != kDSWriteFile) &&
      (command != kDSOpenFile) && (command != kDSCloseFile) &&
      (command != kDSPositionFile)) {
  ...
  if (command == kDSOpenFile) {
    err = HandleOpenFileSupportRequest(
        (DSFileHandle)gTRKCPUState.Default.GPR[4],
        (u8)gTRKCPUState.Default.GPR[5], (u32 *)gTRKCPUState.Default.GPR[6],
        &io_result);
    ...
    err = HandleCloseFileSupportRequest(
        (DSFileHandle)gTRKCPUState.Default.GPR[4], &io_result);
    ...
    // FIXME: No clue why this is causing a build error
    // err = HandlePositionFileSupportRequest(
    //     (DSFileHandle)gTRKCPUState.Default.GPR[4], &msg_length,
    ...
    err = TRK_SuppAccessFile((DSFileHandle)gTRKCPUState.Default.GPR[4],
                             (u8 *)gTRKCPUState.Default.GPR[6], (s32 *)length,
...
PCType TRKTargetGetPC(void) { return (PCType)gTRKCPUState.Default.PC; }
...
DSError TRKTargetAddStopInfo(MessageBuffer *buffer) {
  ...
  TRKTargetReadInstruction((void *)&instruction,
                           (void *)gTRKCPUState.Default.PC);
  buf[3] = instruction;
...
DSError TRKPPCAccessSPR(void *value, u32 spr_register_num, s32 read) {
  InstructionType access_func[10] = {
...
DSError TRKPPCAccessPairedSingleRegister(void *value, u32 register_num,
                                         s32 read) {
  InstructionType access_func[10] = {
      0x60000000, 0x60000000, 0x60000000, 0x60000000, 0x60000000,
      0x60000000, 0x60000000, 0x60000000, 0x60000000, 0x60000000};
  ...
  *registerStorageSize = 0;
  ...
  while ((firstRegister <= lastRegister) && !err) {
    if (read) {
      ...
      err = TRKPPCAccessPairedSingleRegister((void *)&value64, firstRegister,
                                             read);
    }
...
DSError TRKTargetAccessFP(u32 firstRegister, u32 lastRegister, MessageBuffer *b,
                          size_t *registerStorageSize, s32 read) {
  DSError err;
  u32 current;
  FloatType value;
  TRKExceptionStatus savedException;
...
DSError TRKTargetAccessMemory(void *Data, void *virtualAddress,
  ...
  err = TRKValidMemory32(transAddress, *memorySize, read ? 0 : 1);
